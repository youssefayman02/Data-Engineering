...
def append_to_db(cleaned: pd.DataFrame):
    if(engine.connect()):
        print('Connected to Database')
        try:
            print('Appending cleaned dataset to database')
            cleaned.to_sql('fintech_data_MET_1_52_14669_clean', con=engine, if_exists='append')
